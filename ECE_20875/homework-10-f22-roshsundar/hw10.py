...
def conf_matrix(y_pred, y_true, num_class):
    ...
    for i in range(0, len(y_pred)):
        pred = int(y_pred[i])
        actual = int(y_true[i])
        M[pred][actual] += 1 # rows = prediction, cols = actual
    return M
...
def get_model_results(model_name, params, train_data, train_labels, test_data, test_labels, num_class):
    ...
    acc = metrics.accuracy_score(test_labels, response)

    # 5. Calculate the confusion matrix by using the completed the function above
    conf_mat = conf_matrix(response, test_labels, num_class) 

    # 6. Compute the AUROC score. You may use metrics.roc_auc_score(...)
    auc_scor = metrics.roc_auc_score(test_labels, model.predict_proba(test_data), multi_class='ovr')
