...
void setup_tim3(void) {
    ...
    // Configure the PC6-9 to be the outputs of TIM3 Ch 1-4
    // First we clear their MODER bits
    GPIOC -> MODER &= ~(GPIO_MODER_MODER6 | GPIO_MODER_MODER7 | GPIO_MODER_MODER8 | GPIO_MODER_MODER9);
    // Then we set them to AF mode
    GPIOC -> MODER |= (GPIO_MODER_MODER6_1 | GPIO_MODER_MODER7_1 | GPIO_MODER_MODER8_1| GPIO_MODER_MODER9_1);
    // Set PC6-9 to use AF0 since this corresponds to the TIM3 Ch1-4
    // AFR[0] -> AFRL
    GPIOC -> AFR[0] &= ~(GPIO_AFRL_AFR6 | GPIO_AFRL_AFR7);
    // AFR[1] -> AFRH
    GPIOC -> AFR[1] &= ~(GPIO_AFRH_AFR8 | GPIO_AFRH_AFR9);

    // Enable TIM3 with 1 Hz timer
...
void setup_tim1(void) {
    ...
    GPIOA -> AFR[1] |= 0x00002222; // Use AF2
