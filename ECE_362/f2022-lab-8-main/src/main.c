...
void setfreq(float fre)
{
    ...
    // configure TIM2 trigger output
    // triggers on counter update
    TIM2 -> CR2 |= TIM_CR2_MMS_1;

    // trigger DMA with DAC
    DAC -> CR |= DAC_CR_DMAEN1; // triggers DMA channel 3
    ...
    TIM2 -> PSC = (48000000 / (100 * fre * SAMPLES)) - 1;
    TIM2 -> ARR |= 100 - 1;
    TIM2 -> CR1 |= TIM_CR1_CEN; // enable timer

    // DMA setup
    RCC -> AHBENR |= RCC_AHBENR_DMA1EN; // enable DMA clock
    DMA1_Channel3 -> CCR &= ~DMA_CCR_EN; //  turn off
    DMA1_Channel3 -> CPAR = (uint32_t) &(DAC -> DHR12R1); // dest
    DMA1_Channel3 -> CMAR = (uint32_t) (array); // source
    ...
    DMA1_Channel3 -> CCR |= DMA_CCR_EN; // turn on
...
void enable_ports(void) {
    ...
    GPIOB -> MODER &= ~(GPIO_MODER_MODER0 | GPIO_MODER_MODER1 | GPIO_MODER_MODER2 | GPIO_MODER_MODER3| GPIO_MODER_MODER4
                   | GPIO_MODER_MODER5 | GPIO_MODER_MODER6 | GPIO_MODER_MODER7 | GPIO_MODER_MODER8 | GPIO_MODER_MODER9 | GPIO_MODER_MODER10);
    GPIOB -> MODER |= (GPIO_MODER_MODER0_0 | GPIO_MODER_MODER1_0 | GPIO_MODER_MODER2_0 | GPIO_MODER_MODER3_0 | GPIO_MODER_MODER4_0
                | GPIO_MODER_MODER5_0 | GPIO_MODER_MODER6_0 | GPIO_MODER_MODER7_0 | GPIO_MODER_MODER8_0 | GPIO_MODER_MODER9_0 | GPIO_MODER_MODER10_0);
    ...
    GPIOC -> OTYPER &= ~(GPIO_OTYPER_OT_4 | GPIO_OTYPER_OT_5 | GPIO_OTYPER_OT_6 | GPIO_OTYPER_OT_4);
    GPIOC -> OTYPER |= (GPIO_OTYPER_OT_4 | GPIO_OTYPER_OT_5 | GPIO_OTYPER_OT_6 | GPIO_OTYPER_OT_4);
...
void setup_adc(void) {
    ...
    ADC1 -> CHSELR = 0; ADC1 -> CHSELR |= ADC_CHSELR_CHSEL1;
    while(!(ADC1 -> ISR & ADC_ISR_ADRDY));
}
